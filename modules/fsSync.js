...
export function createFsSync({ log, wasm, mountpoint = '/iPod' }) {
    ...
    async function syncIpodToVirtualFS(handle) {
        ...
        // Clean up stale files in MEMFS from previous sessions.
        // itdb_write() creates "Play Counts.bak" in MEMFS, which persists
        // across reconnections.  If itdb_parse() finds it on the next
        // connection, it tries to parse it and logs "Error parsing recent
        // playcounts".  Remove stale files so each session starts clean.
        const itunesVfsDir = `${mountpoint}/iPod_Control/iTunes`;
        {
            let existing;
            try { existing = FS.readdir(itunesVfsDir).filter(n => n !== '.' && n !== '..'); } catch (_) { existing = []; }
            if (existing.length > 0) {
                log(`MEMFS before cleanup: ${existing.join(', ')}`, 'info');
            }
            for (const stale of ['Play Counts', 'Play Counts.bak', 'OTGPlaylistInfo']) {
                try {
                    FS.stat(`${itunesVfsDir}/${stale}`);   // throws if not found
                    FS.unlink(`${itunesVfsDir}/${stale}`);
                    log(`Removed stale ${stale} from MEMFS`, 'info');
                } catch (_) {}
            }
        }
        ...
        // Copy "Play Counts" to MEMFS so libgpod can process it during
        // itdb_parse().  This file contains play counts, ratings, and skip
        // counts written by the iPod firmware.  Without it, libgpod logs
        // "Error parsing recent playcounts".  After parsing, the data is
        // incorporated into the track metadata, so we delete the original
        // from the iPod during syncDbToIpod.
        try {
            const pcHandle = await iTunesHandle.getFileHandle('Play Counts', { create: false });
            const pcFile = await pcHandle.getFile();
            const pcData = new Uint8Array(await pcFile.arrayBuffer());
            FS.writeFile(`${mountpoint}/iPod_Control/iTunes/Play Counts`, pcData);
            log(`Synced: Play Counts (${pcData.length} bytes)`, 'info');
        } catch (_) {
            // No Play Counts file — fine, iPod hasn't recorded any yet
        }
    ...
    async function syncDbToIpod(ipodHandle, { onProgress } = {}) {
        ...
        for (const t of tasks) {
            ...
            if (ok) { syncedCount += 1; syncedFileNames.add(t.fileName); }
            done += 1;
        }

        // If iTunesSD was NOT regenerated by libgpod (not in MEMFS), delete the
        // old copy from the iPod.  A stale iTunesSD contains an outdated track
        // list that the iPod firmware uses for certain UI screens (music browser,
        // mode selection).  This causes the iPod to display the wrong track count
        // even though iTunesDB is correct.  Removing the stale file forces the
        // firmware to rely solely on iTunesDB for all screens.
        if (!syncedFileNames.has('iTunesSD')) {
            try {
                await iTunesHandle.removeEntry('iTunesSD');
                log('Removed stale iTunesSD from iPod (not regenerated by libgpod)', 'info');
            } catch (_) {
                // iTunesSD didn't exist on iPod either — nothing to remove
            }
        }

        // Delete stale files from the iPod that could cause wrong track counts
        // or confuse the firmware:
        //
        // - "Play Counts": firmware writes play counts/ratings/skip counts here.
        //   iTunes reads and deletes it during sync. Stale data can cause the
        //   firmware to use an outdated track count in the Music menu.
        //
        // - "OTGPlaylistInfo": on-the-go playlist data written by firmware.
        //
        // - "iTunesCDB": compressed database used by Nano 5G+. If we're using
        //   classic layout (iTunesDB), a leftover iTunesCDB could cause the
        //   firmware to read from the wrong database on devices that check for it.
        for (const staleFile of ['Play Counts', 'OTGPlaylistInfo', 'iTunesCDB']) {
            try {
                await iTunesHandle.removeEntry(staleFile);
                log(`Removed stale ${staleFile} from iPod`, 'info');
            } catch (_) {
    ...
    async function syncVirtualFileToRealInternal(realDirHandle, virtualPath, fileName, optional = false) {
        ...
        try {
            ...
            // IMPORTANT: FS.readFile may return a view of the internal MEMFS
            // buffer rather than a copy.  We must copy it to an independent
            // ArrayBuffer before passing it to the async File System Access API
            // write, so the data remains valid throughout the write operation.
            const raw = FS.readFile(virtualPath);
            ...
            log(`Synced ${fileName} to iPod (${data.length} bytes)`, 'info');
            return true;
        } catch (e) {
